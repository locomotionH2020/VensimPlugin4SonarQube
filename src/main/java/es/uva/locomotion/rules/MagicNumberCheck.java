...
public class MagicNumberCheck extends VensimCheck {
    ...
    @RuleProperty(
            key = "ignored-numbers",
            defaultValue = DEFAULT_IGNORED_NUMBERS,
            description = "Numbers that are not counted as magic numbers. Separate the list of numbers with ';'")
    public String ignoredNumbers = DEFAULT_IGNORED_NUMBERS;


    private List<String> getIgnoredNumbers() {
        try {
            return Arrays.stream(ignoredNumbers.split(";")).collect(Collectors.toList());
        } catch (Exception ex) {
            // Empty catch block so that the error is logged if 'ignoredNumbers' have an invalid structure.
        }
        logger.unique("The rule " + NAME + " has an invalid configuration: The selected minimum repetitions must be a number greater than 0.",
                LoggingLevel.ERROR);
        return Arrays.stream(DEFAULT_IGNORED_NUMBERS.split(";")).collect(Collectors.toList());
    ...
    public void scan(VensimVisitorContext context) {
        ...
        for (Number symbol : numberTable.getNumbers()) {
            ...
            if (foundRepetitions >= 1 && !getIgnoredNumbers().contains(symbol.getToken().trim())) {
